...
<c:if test="${not empty mealDates}">
    <div class="form-group">
        ...
        <form method="post" action="show_diet">
            <input type="hidden" name="command" value="show_diet">
            <input type="hidden" name="userIdForSupervisor" value="${userIdForSupervisor}">

            <select name="mealDate" onchange="this.form.submit()" class="selectpicker" id="exampleFormControlSelect1">
...
<c:if test="${not empty breakfastMeals or not empty lunchMeals or not empty dinnerMeals}">
    ...
    <c:if test="${ok != null}">
        <span style="color:green;"><fmt:message key="profile.message"/></span>
    </c:if>
    ...
    <table class="table">
                        ...
                        <c:when test="${not dietOfUserForSupervisor}">
                            <td>
                                <form method="post" action="update_quantity_in_diet">
                                    ...
                                    <input type="hidden" name="mealDate" value="${breakfastMeal.date}">

                                    <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                            <span class="input-group-text"><fmt:message key="product.quantity"/></span>
                                        </div>
                                        <input type="number" min="1" name="quantity" value="${breakfastMeal.quantity}"
                                               maxlength="10"
                        ...
                        <c:when test="${not dietOfUserForSupervisor}">
                            <td>
                                <form method="post" action="update_quantity_in_diet">
                                    ...
                                    <input type="hidden" name="mealDate" value="${lunchMeal.date}">

                                    <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                <span class="input-group-text" id="inputGroup-sizing-default"><fmt:message
                                        key="product.quantity"/></span>
                                        </div>
                                        <input type="number" min="1" name="quantity" value="${lunchMeal.quantity}"
                                               maxlength="10"
                                               class="form-control" aria-label="Sizing example input"
                                               aria-describedby="inputGroup-sizing-default">
                                        <span style="color:red;">${errorQuantity}</span>
        ...
        <c:if test="${not empty dinnerMeals}">
            <tr>
                <th rowspan="${dinnerMeals.size() + 1}" scope="rowgroup"><fmt:message key="product.dinner"/></th>
            </tr>
                        ...
                        <c:when test="${not dietOfUserForSupervisor}">
                            <td>
                                <form method="post" action="update_quantity_in_diet">
                                    ...
                                    <input type="hidden" name="mealDate" value="${dinnerMeal.date}">

                                    <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                            <span class="input-group-text"><fmt:message key="product.quantity"/></span>
                                        </div>
                                        <input type="number" min="1" name="quantity" value="${dinnerMeal.quantity}"
                                               maxlength="10"
                                               class="form-control" aria-label="Sizing example input"
                                               aria-describedby="inputGroup-sizing-default">
                                        <span style="color:red;">${errorQuantity}</span>
            ...
            <div class="col-sm-10 col-sm-offset-1">
                ...
                <div class="comment-tabs">
                    <ul class="nav nav-tabs" role="tablist">
                        <li class="nav-item nav-link active"><a href="#comments-logout" role="tab" data-toggle="tab"
                                                                style="text-decoration:none"><h4
                                class="reviews text-capitalize"><fmt:message key="profile.comments"/></h4></a></li>
                    ...
                    <div class="tab-content">
                                        ...
                                        <div class="media-body" style="padding-bottom: 15px;">
                                            <c:if test="${User.userId == commentForUser.commentator.userId or User.userId == commentForUser.userId}">
                                                <form method="post" action="delete_comment">
                                                    ...
                                                    <input type="hidden" name="commentId" value="${commentForUser.commentId}">
                                            ...
                                            <div class="card card-body bg-light">
                                                <div>
                                                    <h4 class="media-heading text-uppercase" style="display:inline-block">
                                                            ${commentForUser.commentator.firstName} ${commentForUser.commentator.surname}
                                                    </h4>
                                                    <span class="float-right" style="display:inline-block">${commentForUser.dateOfComment}</span>
                        ...
                        <div class="tab-pane" id="add-comment">
                            <form method="post" action="comment" class="form-horizontal" id="commentForm" role="form">
                                ...
                                <div class="form-group">
                                    <div class="col-sm-10">
                                    <textarea minlength="1" maxlength="2000" class="form-control" name="comment" id="addComment"
                                              rows="5"></textarea>
                                    </div>
                                </div>
